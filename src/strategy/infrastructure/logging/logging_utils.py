...
def _resolve_fallback_level() -> int:
    ...
    level_name = os.getenv("STRATEGY_FALLBACK_LOG_LEVEL", "INFO").strip().upper()
...
def setup_strategy_logger(name: str, log_file: str = "strategy.log") -> logging.Logger:
    ...
    fallback_level = _resolve_fallback_level()
    ...
    file_handler.setLevel(fallback_level)
    logger.addHandler(file_handler)
    
    # 控制台处理器
    console_handler = logging.StreamHandler(sys.stdout)
    console_handler.setFormatter(formatter)
    console_handler.setLevel(fallback_level)
